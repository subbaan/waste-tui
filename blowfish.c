/*
    WASTE - blowfish.c (Blowfish cipher)
...
static void bfe(BLOWFISH_CTX *ctx, uint32_t *xl, uint32_t *xr) 
{
  uint32_t Xl=*xl;
  uint32_t Xr=*xr;
  int i=N/2;
...
static void bfd(BLOWFISH_CTX *ctx, uint32_t *xl, uint32_t *xr) 
{
  uint32_t Xl=*xl;
  uint32_t Xr=*xr;
  uint32_t *p=ctx->P + N + 1;
  uint32_t *s=(uint32_t *)ctx->S;
  int i=N/2;
  while (i--)
  {
...
#define BSWAP(x)
#define bfe Blowfish_Encrypt
#define bfd Blowfish_Decrypt
...
void Blowfish_Init(BLOWFISH_CTX *ctx, unsigned char *key, int keyLen) {
  ...
  memcpy(ctx->S,ORIG_S,256*4*sizeof(uint32_t));
