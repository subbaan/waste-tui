...
class XferSend
{
  public:    
   ...
   ~XferSend();
    ...
    // Progress getters for TUI
    unsigned int getChunksSent() const { return m_chunks_sent_total; }
    unsigned int getChunksTotal() const { return m_filelen_chunks; }
    unsigned int getMaxChunkSent() const { return m_max_chunksent; }
    int getSpeedCps() const { return m_last_cps; }
...
class XferRecv
{
  public:
    ...
    // Progress getters for TUI
    unsigned int getChunkCount() const { return m_chunk_cnt; }
    unsigned int getChunkTotal() const { return m_chunk_total; }
    unsigned int getBytesTotalLow() const { return m_bytes_total_l; }
    unsigned int getBytesTotalHigh() const { return m_bytes_total_h; }
    int getSpeedCps() const { return m_last_cps; }
    bool isDone() const { return m_done != 0; }
