...
XferRecv::XferRecv(C_MessageQueueList *mql, char *guididx, char *sizestr, char *filename, char *path)
{
  ...
  do
  {
    ...
    sprintf(m_statfile_fn,"%s.WASTESTATE",m_outfile_fn);
#ifdef XFER_WIN32_FILEIO
  ...
  if (m_statfile &&    
    ...
    (sig1 == WASTESTATE_SIG1) && (sig2 == WASTESTATE_SIG2) &&
...
XferRecv::~XferRecv()
{
  debug_printf("[XFER] ~XferRecv: m_done=%d statfile_fn='%s'\n", m_done, m_statfile_fn ? m_statfile_fn : "(null)");
  ...
  if (m_done)
  {
    ...
    {
      debug_printf("[XFER] Deleting state file: %s\n", m_statfile_fn);
      int delResult = DeleteFile(m_statfile_fn);
      debug_printf("[XFER] DeleteFile result: %d (errno=%d)\n", delResult, errno);
    }
    else
    {
      debug_printf("[XFER] NOT deleting state file: m_statfile=%p fn='%s'\n",
        (void*)m_statfile, m_statfile_fn ? m_statfile_fn : "(null)");
    }
  }
