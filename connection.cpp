...
C_Connection::C_Connection(char *hostname, int port, C_AsyncDNS *dns)
{
  ...
  else
  {
    ...
    if (!safe_inet_pton(hostname, &m_saddr.sin_addr)) {
...
void C_Connection::init_crypt()
{
  ...
  // according to X9.17, afaik.
  {
    ...
    int x;
    uint32_t *t=(uint32_t *)m_send_buffer;
  ...
  if (g_use_networkhash)
  {
    ...
    {
      Blowfish_Encrypt(&c,(uint32_t *)(m_send_buffer+CONNECTION_BFPUBKEY+x),
            (uint32_t *)(m_send_buffer+CONNECTION_BFPUBKEY+x+4));
    }
    memset(&c,0,sizeof(c));
  }
...
void C_Connection::init_crypt_gotpkey() // got their public key
{
  ...
  for (x = 0; x < CONNECTION_PKEYHASHSIZE; x += 8)
  {
    Blowfish_Decrypt(&c,(uint32_t *)(m_remote_pkey+x),(uint32_t *)(m_remote_pkey+x+4));        
  }
  ...
  {
    debug_printf("connection: remote public key not found in keyring, rejecting connection\n");
    close(1);
    return;
  }
  ...
  // according to X9.17, afaik.
  {
    ...
    int x;
    uint32_t *t=(uint32_t *)m_mykeyinfo;
  ...
  if (g_use_networkhash)
  {
    ...
    {
      Blowfish_Encrypt(&c,(uint32_t *)(localkeyinfo+x),(uint32_t *)(localkeyinfo+x+4));
    }
    memset(&c,0,sizeof(c));
  }
...
void C_Connection::init_crypt_decodekey() // got their encrypted session key for me to use
{
  ...
  if (g_use_networkhash)
  {
    ...
    for (x = 0; x < CONNECTION_KEYSIZE; x += 8)
    {
      Blowfish_Decrypt(&c,(uint32_t *)(m_key+x),(uint32_t *)(m_key+x+4));
    }
    memset(&c,0,sizeof(c));
  }
...
C_Connection::state C_Connection::run(int max_send_bytes, int max_recv_bytes)
{
  ...
  {		
    ...
    memset(&tv,0,sizeof(tv));
		if (select(SELECT_NFDS(m_socket),NULL,&f,&f2,&tv)==-1)
		{
...
//    MessageBox(NULL,buf,APP_NAME " BIG BIG ERROR",0);
  ...
  for (x = 0; x < len; x += 8) 
  {
    uint32_t *p=(uint32_t*)(t+x);
    uint32_t o1=p[0], o2=p[1];
