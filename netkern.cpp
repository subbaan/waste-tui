...
void NetKern_ConnectToHostIfOK(int ip, int port)
{
  ...
  in.s_addr=ip;
  char *t=inet_ntoa(in);
  debug_printf("NetKern_ConnectToHostIfOK: ip=%08X port=%d inet_ntoa=%s\n", ip, port, t ? t : "(null)");
  if (!t || !*t) {
    debug_printf("NetKern_ConnectToHostIfOK: inet_ntoa failed, returning\n");
    return;
  }
  char text[512];
  ...
  int acc = isaccessable(text);
  int allow = allowIP(ip);
  debug_printf("NetKern_ConnectToHostIfOK: isaccessable=%d allowIP=%d\n", acc, allow);
  if (acc && allow)
  {
    C_Connection *newcon=new C_Connection(text,port,g_dns);
    debug_printf("NetKern_ConnectToHostIfOK: created connection, sending sync, adding to g_new_net\n");
...
static void HandleNewOutCons(void)
{
  ...
  for (x = 0; x < g_new_net.GetSize(); x ++)
  {
    ...
    if (s != last_log_state) {
      debug_printf("HandleNewOutCons: connection %d state=%d\n", x, s);
      last_log_state = s;
    }
