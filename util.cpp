...
static int readBFdata(FILE *in, BLOWFISH_CTX *ctx, void *data, unsigned int len, uint32_t *cbc)
{
  unsigned int x;
  for (x = 0; x < len; x++)
  {
...
//            tl[x/4]|=c<<((x&3)*8);
        ...
        {
          char buf[8];
          if (readBFdata(fp,&bf,buf,8,(uint32_t *)tl)) err="Private key corrupt";
          ...
          {
            int a=0;
          #define WPK(x) if (!readBFdata(fp,&bf,g_key.x,sizeof(g_key.x),(uint32_t *)tl))
            WPK(modulus)
            WPK(publicExponent)
            WPK(exponent)
            WPK(prime)
