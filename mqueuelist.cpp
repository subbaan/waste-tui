...
C_MessageQueueList::C_MessageQueueList(void (*gm)(T_Message *message, C_MessageQueueList *_this, C_Connection *cn), int maxttl)
{
  m_got_message=gm;
...
int C_MessageQueueList::send(T_Message *msg)
{
  ...
  if (MESSAGE_TYPE_ROUTED(msg->message_type)) // routed
  {
    ...
    if (a >= 0)
    {
      ...
      m_queues->Get(a)->send_message(msg);
    }
    else if (a == -1)
    {
      debug_printf("queuelist::send(): warning: tried to send routed message to self!\n");
    }
    else 
    {
      m_stat_route_errors++;
      debug_printf("queuelist::send(): no route found for message of type %d!\n",msg->message_type);
    }
  }
  else if (MESSAGE_TYPE_LOCAL(msg->message_type))
  {
    debug_printf("queuelist::send(): warning: tried to send local message to list!\n");
  }
  else // broadcast
  {
    ...
    {
      m_queues->Get(o)->send_message(msg);
    }
  }
  msg->data->Unlock();
  return 0;
...
void C_MessageQueueList::run(int doRouting)
{
  ...
  {
    m_queues->Get(runcnt)->get_con()->calc_bps(&sbps,&rbps);
    total_sendbw+=sbps;
    total_recvbw+=rbps;
    runcnt++;
  }
  ...
  while (runcnt < numqueues)
  {
    ...
    C_Connection *con=m_queues->Get(thisq)->get_con();
    ...
    if (sbps!=0) m_queues->Get(thisq)->run(0,sbps);
    int s=con->run(sbps,rbps);

    for (;;)
    {
      T_Message msg={0,};
      m_queues->Get(thisq)->run(1,-1);
      if (m_queues->Get(thisq)->recv_message(&msg)) break;
      ...
      if (MESSAGE_TYPE_ROUTED(msg.message_type))
      {
        ...
        {          
          ...
          {
            msg.message_ttl--;
            if (msg.message_ttl>0) m_queues->Get(r)->send_message(&msg);
          }
        }
        else if (r==-1)// this one was meant for us!
        {
          m_got_message(&msg,this,m_queues->Get(thisq)->get_con());
        }
        else 
        {
          m_stat_route_errors++;          
          debug_printf("queuelist::run(): no route found for relaying message!\n");
        }
      }
      else if (MESSAGE_TYPE_LOCAL(msg.message_type))
      {
      ...
      {
        int r=find_route(&msg.message_guid,msg.message_type);
        if (msg.message_type == MESSAGE_CHAT || msg.message_type == MESSAGE_CHAT_REPLY) {
          debug_printf("[CHAT-MQ] Broadcast msg type=%d, find_route=%d (need <-1 to process)\n",
            msg.message_type, r);
        }
        if (r < -1)
        {
          int o;
          m_queues->Get(thisq)->add_route(&msg.message_guid,msg.message_type);
          if (msg.message_ttl>1 && doRouting && (msg.message_type != MESSAGE_KEYDIST || (g_keydist_flags&4)))
          {
            ...
            {
              if (o != thisq) m_queues->Get(o)->send_message(&msg);
            }
          }
          m_got_message(&msg,this,m_queues->Get(thisq)->get_con());
        }
        else if (msg.message_type == MESSAGE_CHAT || msg.message_type == MESSAGE_CHAT_REPLY) {
          debug_printf("[CHAT-MQ] DROPPED as duplicate (r=%d)\n", r);
        }
      }
    ...
    if (s == C_Connection::STATE_ERROR || s == C_Connection::STATE_CLOSED)
    {
      ...
      if (m_lv)
      {
        int i=m_lv->FindItemByParam((int)m_queues->Get(thisq)->get_con());
        if (i != -1)
        {
          char text[512];
          ...
          else
          {
            ...
            if (!m_queues->Get(thisq)->get_stat_recv() || !m_queues->Get(thisq)->get_stat_send())
            {
...
int C_MessageQueueList::find_route(T_GUID *id, unsigned char msgtype)
{
  int r;
  if (m_local_route->is_route(id,msgtype)) return -1;
  for (r = 0; r < m_queues->GetSize(); r ++)
    if (m_queues->Get(r)->is_route(id,msgtype))
      return r;
 return -2;
}
